...
def upload_to_firebase(csv_file_path):
    ...
    # Upload the sanitized CSV data to Firebase
    for _, row in df.iterrows():
        """sanitized_row = {
            'timestamp': row['timestamp'],
            'x_position': sanitize_data(row['x_position']),
            'y_position': sanitize_data(row['y_position']),
            'button': row['button'],
            'click': row['click'],
            'dx': sanitize_data(row['dx']),
            'dy': sanitize_data(row['dy']),
            'key': row['key'],
            'press': row['press']
        }"""
        for _,row in df.iterrows(): 
